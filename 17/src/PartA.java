...
public class PartA {
	public static void main(String[] args) {
        ...
        while (it < 6) {
        	...
        	for (String id : map) {
        		...
        		int nActive = getActiveNeighbours(map, coords[0], coords[1], coords[2], candidates);
    		...
    		for (String candidateId : candidates) {
        		...
        		if (getActiveNeighbours(map, coords[0], coords[1], coords[2], null) == 3) {
	...
	private static int getActiveNeighbours(HashSet<String> map, int x, int y, int z, HashSet<String> pendingCandidates) {
		int nActive = 0;
		for (int i = -1; i <= 1; i++) {
			for (int j = -1; j <= 1; j++) {
				for (int k = -1; k <= 1; k++) {
					...
					if (map.contains(getID(x + j, y + i, z + k))) nActive++;
		    		else if (pendingCandidates != null) pendingCandidates.add(getID(x + j, y + i, z + k));
				}
			}
		}
		return nActive;
